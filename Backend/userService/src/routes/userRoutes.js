...
function attachCategoryIcon(project = {}, iconMap = null) {
  const globalCategory = normalizeGlobalCategory(project.global_category);
  const icon = iconMap?.get(globalCategory) || project;
  return {
    ...project,
    global_category: globalCategory || project.global_category || "",
    category_icon_url: project.category_icon_url || categoryIconDisplayUrl(icon),
    category_background_url: project.category_background_url || categoryBackgroundDisplayUrl(icon)
  };
}

async function loadProjectCategoryIconMap() {
  await ensureCoinSchema();
  const { rows } = await pool.query(
    `SELECT global_category, image_url, image_public_id, mime_type, background_image_url, background_image_public_id, background_mime_type
     FROM project_category_icons`
  );
  return new Map(rows.map((row) => [row.global_category, row]));
}
...
function structuredStagesFromStep(step = {}) {
  ...
  return stages
    ...
    .filter((stage) => stage.title || stage.github_integration_required || stage.demo_document_id || stage.demo_document_ids.length);
}
...
function parseStagesFromStepContext(text) {
  ...
  if (!stagesRaw) return [];
  return splitStageBlocks(stagesRaw)
    .map((block) => {
...
function selectedDemoDocumentIdsFromSteps(steps = []) {
  ...
  for (const step of steps || []) {
    for (const stage of structuredStagesFromStep(step)) {
      if (stage.demo_document_id) ids.add(stage.demo_document_id);
      for (const id of stage.demo_document_ids || []) ids.add(id);
    }
...
function countStepUnits(step) {
  const structuredStages = structuredStagesFromStep(step);
  const stages = structuredStages.length ? structuredStages : parseStagesFromStepContext(step?.step_context);
...
async function hasConnectedGithubRepository(userId) {
  try {
...
async function listActiveCatalogProjects({ limit = 50, includeDemoProjects = false, onlyDemoProjects = false } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT p.id, p.title, p.description, p.category, p.global_category, p.timeline_weeks, p.project_coins, p.is_demo_project,
            p.complexity, p.keywords, p.skills_required, p.skills_gained, p.target_branch, p.target_year,
            p.tech_stack, p.short_summary, p.domain, p.prerequisites, p.learning_outcomes, p.difficulty_score,
            pci.image_url AS category_icon_raw_url,
            pci.image_public_id AS category_icon_public_id,
            pci.mime_type AS category_icon_mime_type,
            pci.background_image_url AS category_background_raw_url,
            pci.background_image_public_id AS category_background_public_id,
            pci.background_mime_type AS category_background_mime_type
     FROM projects p
     LEFT JOIN project_category_icons pci ON pci.global_category = p.global_category
     WHERE p.is_active = TRUE
       AND ($2::boolean = TRUE OR COALESCE(p.is_demo_project, FALSE) = FALSE)
       AND ($3::boolean = FALSE OR COALESCE(p.is_demo_project, FALSE) = TRUE)
...
function scoreProjects({ branch, year, resumeUrl, projects }) {
  ...
  const scored = (projects || []).map((p) => {
    ...
    return {
      ...
      category_icon_url: p.category_icon_url || categoryIconDisplayUrl({ image_url: p.category_icon_raw_url, image_public_id: p.category_icon_public_id, mime_type: p.category_icon_mime_type }),
      category_background_url: p.category_background_url || categoryBackgroundDisplayUrl({ background_image_url: p.category_background_raw_url, background_image_public_id: p.category_background_public_id, background_mime_type: p.category_background_mime_type }),
...
function publicRecommendationProject(project = {}) {
  return {
    ...
    category_icon_url: project.category_icon_url || "",
    category_background_url: project.category_background_url || "",
...
function buildCatalogProjectLookup(projects = []) {
  const lookup = new Map();
  for (const project of projects || []) {
    const id = Number(project?.id);
    if (Number.isFinite(id) && id > 0) lookup.set(`id:${id}`, project);
    const name = String(project?.name || project?.title || "").trim().toLowerCase();
    if (name) lookup.set(`name:${name}`, project);
  }
  return lookup;
}

function enrichRecommendationProject(project = {}, catalogLookup = new Map()) {
  const id = Number(project?.id);
  const name = String(project?.name || project?.title || "").trim().toLowerCase();
  const catalogProject = (Number.isFinite(id) && catalogLookup.get(`id:${id}`)) || (name && catalogLookup.get(`name:${name}`)) || null;
  if (!catalogProject) return project;
  return {
    ...catalogProject,
    ...project,
    global_category: project.global_category || catalogProject.global_category || "",
    category_icon_url: project.category_icon_url || catalogProject.category_icon_url || "",
    category_background_url: project.category_background_url || catalogProject.category_background_url || ""
  };
}
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const catalogLookup = buildCatalogProjectLookup(catalog);
    const iconMap = await loadProjectCategoryIconMap();
    try {
      ...
      if (projects.length) {
        ...
        return res.json({
          offset,
          projects: projects.slice(0, onlyDemoProjects ? 50 : 2).map((project) => publicRecommendationProject(attachCategoryIcon(enrichRecommendationProject(project, catalogLookup), iconMap))),
          source: "recommendation_service"
        });
      }
    } catch {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { rows } = await pool.query(
      `SELECT p.id, p.title, p.description, p.category, p.global_category, p.timeline_weeks,
              p.project_coins,
              p.is_demo_project,
              p.introduction_document, p.introduction_document_url,
              p.company_profile_text,
              p.solution_document, p.solution_document_url,
              p.steps_json,
              pci.image_url AS category_icon_raw_url,
              pci.image_public_id AS category_icon_public_id,
              pci.mime_type AS category_icon_mime_type,
    ...
    return res.json({
      project: {
        ...
        category: p.category,
        global_category: p.global_category || "",
        category_icon_url: categoryIconDisplayUrl({ image_url: p.category_icon_raw_url, image_public_id: p.category_icon_public_id, mime_type: p.category_icon_mime_type }),
        category_background_url: categoryBackgroundDisplayUrl({ background_image_url: p.category_background_raw_url, background_image_public_id: p.category_background_public_id, background_mime_type: p.category_background_mime_type }),
        timeline_weeks: p.timeline_weeks,
        project_coins: Number(p.project_coins) || 1,
        is_demo_project: Boolean(p.is_demo_project),
    ...
    return res.json({
      projects: projectSummaries
        .map(({ row, summary }) => ({
          ...
          category_icon_url: categoryIconDisplayUrl({ image_url: row.category_icon_raw_url, image_public_id: row.category_icon_public_id, mime_type: row.category_icon_mime_type }),
          category_background_url: categoryBackgroundDisplayUrl({ background_image_url: row.category_background_raw_url, background_image_public_id: row.category_background_public_id, background_mime_type: row.category_background_mime_type }),
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const projectRes = await client.query(
        `SELECT p.id, p.title, COALESCE(p.project_coins, 1) AS project_coins, COALESCE(p.is_demo_project, FALSE) AS is_demo_project
         FROM projects p
         WHERE p.is_active = TRUE
           AND LOWER(TRIM(p.title)) = LOWER(TRIM($1))
         LIMIT 1`,
        [projectName]
      );
  ...
  asyncHandler(async (req, res) => {
    ...
    if (status === "completed") {
      if (await stageRequiresGithubIntegration({ projectName, stepNumber, stageIndex })) {
        const githubConnected = await hasConnectedGithubRepository(userId);
        if (!githubConnected) {
